...
namespace Qlns
{
    public partial class NhanVien_ThemNhanVien1 : Form
    {
        ...
        private void NhanVien_ThemNhanVien1_Load(object sender, EventArgs e)
        {
            ...
            try
            {
                using (connection = kn.OpenConnection())
                {
                    string query = "SELECT * FROM CongTac where CongTac.Status = '1'";
                    cmd = new SqlCommand(query, connection);
                    adapter = new SqlDataAdapter(cmd);
                    DataTable dataTable = new DataTable();
                    adapter.Fill(dataTable);
